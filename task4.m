...
% % Using points of correspondence from Task 3
% points_img1 = [floor_1; Door_1; Wall_1];
% points_img2 = [floor_2; Door_2; Wall_2];
% 
% % Extract x and y coordinates for the two images
% x1 = points_img1(:, 1);
% y1 = points_img1(:, 2);
% x2 = points_img2(:, 1);
% y2 = points_img2(:, 2);
% 
% % Visualize the selected points on the first image
% figure(1); imagesc(im); axis image; hold on;
% for i=1:length(x1)
%    h = plot(x1(i), y1(i), '*');
...
% hold off;
% 
% % Visualize the selected points on the second image
% figure(2); imagesc(im2); axis image; hold on;
% for i=1:length(x2)
%    h = plot(x2(i), y2(i), '*');
%    set(h, 'Color', 'g', 'LineWidth', 2);
